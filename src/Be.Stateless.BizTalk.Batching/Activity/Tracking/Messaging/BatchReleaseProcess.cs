...
namespace Be.Stateless.BizTalk.Activity.Tracking.Messaging
{
	...
	internal class BatchReleaseProcess : Process
	{
		...
		{
			if (name.IsNullOrEmpty()) throw new ArgumentException("Process name is null or empty.", nameof(name));
			ProcessName = name;
		}
		...
		[SuppressMessage("ReSharper", "VirtualMemberNeverOverridden.Global", Justification = "virtual for mocking purposes.")]
		public virtual void AddSteps(IEnumerable<string> messagingStepActivityIdCollection)
		{
			messagingStepActivityIdCollection.ForEach(
